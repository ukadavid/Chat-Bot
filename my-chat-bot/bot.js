...
class MyBot extends ActivityHandler {
    constructor() {
        ...
        this.onMessage(async (context, next) => {
            // Check if the user provided the authentication number
            if (context.activity.text === '10112234') {
                // User is authenticated
                // You can store the authentication state here

                // Get user information if available
                const userName = context.activity.from.name;
                const userEmail = context.activity.from.email;

                await context.sendActivity(`Authentication successful! Welcome, ${ userName } (${ userEmail }).`);
                await this.displayMainMenu(context);
                return;
            } else if (!context.activity.from.name || !context.activity.from.email) {
                // User is not authenticated, request authentication
                await context.sendActivity('You are not authenticated. Please provide your name and email.');
                return;
            }
            ...
            if (text === 'menu') {
                await this.displayMainMenu(context, options);
            } else if (options.includes(text)) {
                await context.sendActivity(`You selected: ${ text }`);
            } else {
                await context.sendActivity('Invalid option. Please select a valid option or type "menu" to see the main menu.');
            }
    ...
    async displayMainMenu(context, options) {
        const cardActions = options.map((option) => ({
        ...
        const card = CardFactory.adaptiveCard({
            ...
            body: [
                ...
                {
                    type: 'TextBlock',
                    text: 'Welcome to XYZ, what can I do for you today?',
                    wrap: true
                }
            ],
            actions: cardActions
